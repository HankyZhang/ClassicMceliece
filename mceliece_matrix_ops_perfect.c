...
// Matrix creation and basic operations (unchanged)
...
// Perfect matrix building following reference pk_gen.c exactly
int build_parity_check_matrix_perfect(matrix_t *H, const polynomial_t *g, const gf_elem_t *support) {
    ...
    // Compute inv array exactly like reference
    gf_elem_t *inv = malloc(SYS_N * sizeof(gf_elem_t));
    if (!inv) return -1;
    
    printf("Computing inv array (g^-1 at support points)...\n");
    for (int j = 0; j < SYS_N; j++) {
        gf_elem_t eval = polynomial_eval(g, support[j]);
        if (eval == 0) {
            printf("❌ Zero evaluation at support[%d]\n", j);
            free(inv);
            return -1;
        }
        inv[j] = gf_inv(eval);
    ...
    printf("Building matrix exactly like reference...\n");
    
    // Reference matrix building algorithm - EXACT COPY
    for (int i = 0; i < SYS_T; i++) {
        for (int j = 0; j < SYS_N; j += 8) {
            for (int k = 0; k < GFBITS; k++) {
                uint8_t b = 0;
                
                // Exact reference bit packing (MSB = highest column)
        ...
        // Update inv for next iteration: inv[j] *= support[j]
        for (int j = 0; j < SYS_N; j++) {
    ...
    free(inv);
    printf("✅ Matrix built using exact reference algorithm\n");
    return 0;
}

// Perfect Gaussian elimination following reference pk_gen.c exactly
int reduce_to_systematic_form_perfect(matrix_t *H) {
    ...
    printf("Starting perfect Gaussian elimination (exact reference)...\n");
    printf("Matrix: %d x %d\n", PK_NROWS, SYS_N);
    
    // EXACT COPY of reference Gaussian elimination
    for (int i = 0; i < (PK_NROWS + 7) / 8; i++) {
        for (int j = 0; j < 8; j++) {
            ...
            // Forward elimination step: eliminate below diagonal
            for (int k = row + 1; k < PK_NROWS; k++) {
                // Extract bit from current row and target row
                uint8_t mask = H->data[row * H->cols_bytes + i] ^ H->data[k * H->cols_bytes + i];
                mask >>= j;
                mask &= 1;
                mask = (uint8_t)(-(int8_t)mask);  // Sign extend to all bits
            ...
            // Check if diagonal element is 1 (required for systematic form)
            if (((H->data[row * H->cols_bytes + i] >> j) & 1) == 0) {
                printf("❌ Matrix not systematic at row %d, bit %d\n", row, j);
                return -1;  // Not systematic
            }
            
            // Backward elimination step: eliminate above and below diagonal
    ...
    printf("✅ Perfect Gaussian elimination completed\n");
    return 0;
}
...
int matrix_is_systematic(const matrix_t *mat) {
    ...
    // Check if the leading square submatrix is identity
...
int extract_public_key_from_systematic_matrix(const matrix_t *H_sys, uint8_t *pk_out) {
    if (!H_sys || !pk_out) return -1;
    
    int PK_NROWS = H_sys->rows;
    int SYS_N = H_sys->cols;
    int PK_ROW_BYTES = (SYS_N - PK_NROWS + 7) / 8;  // Non-identity part
    
    printf("Extracting public key from systematic matrix...\n");
    printf("PK dimensions: %d x %d bytes per row\n", PK_NROWS, PK_ROW_BYTES);
    
    // Copy the non-identity part (right side) of the systematic matrix
    for (int i = 0; i < PK_NROWS; i++) {
        uint8_t *src = &H_sys->data[i * H_sys->cols_bytes + PK_NROWS / 8];
        uint8_t *dst = &pk_out[i * PK_ROW_BYTES];
        memcpy(dst, src, PK_ROW_BYTES);
    }
...
// Finite field matrix operations
