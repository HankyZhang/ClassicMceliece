...
)
{
	...
	{
		attempts++;
		printf("DEBUG: Reference implementation attempt %d\n", attempts);
		rp = &r[ sizeof(r)-32 ];
		skp = sk;
		...
		if (genpoly_gen(irr, f)) {
			printf("DEBUG: genpoly_gen failed on attempt %d\n", attempts);
			continue;
		}
		...
		if (pk_gen(pk, skp - IRR_BYTES, perm, pi)) {
			printf("DEBUG: pk_gen failed on attempt %d\n", attempts);
			continue;
		}
		...
		printf("DEBUG: Reference implementation SUCCESS after %d attempts\n", attempts);
		break;
	}
